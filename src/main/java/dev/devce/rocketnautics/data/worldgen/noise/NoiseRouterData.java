...
public class NoiseRouterData {
    ...
    public static final ResourceKey<DensityFunction> MOON_PRIMARY = register("moon/primary");
    ...
    public static void bootstrap(BootstrapContext<DensityFunction> context) {
        ...
        DensityFunction continents = registerAndWrap(context, MOON_CONTINENTS, DensityFunctions.flatCache(DensityFunctions.shiftedNoise2d(shiftX, shiftZ, 0.25, noises.getOrThrow(NoiseData.MOON_CONTINENTS))));
        DensityFunction ridges = registerAndWrap(context, MOON_RIDGES, DensityFunctions.flatCache(DensityFunctions.shiftedNoise2d(shiftX, shiftZ, 0.25, noises.getOrThrow(NoiseData.MOON_RIDGES))));
        DensityFunction ridgesFolded = registerAndWrap(context, MOON_RIDGES_FOLDED, peaksAndValleys(ridges));
        DensityFunction wrinkleRidges = registerAndWrap(context, MOON_WRINKLE_RIDGES, DensityFunctions.flatCache(new StretchedNoise(new DensityFunction.NoiseHolder(noises.getOrThrow(NoiseData.MOON_WRINKLE_RIDGES)), 1, 0.1, 0.1)));
        context.register(MOON_EROSION, DensityFunctions.flatCache(DensityFunctions.shiftedNoise2d(shiftX, shiftZ, 0.25, noises.getOrThrow(NoiseData.MOON_EROSION))));
    ...
    public static NoiseRouter moon(HolderGetter<NormalNoise.NoiseParameters> noises, HolderGetter<DensityFunction> densities) {
        ...
        var ridgesFolded = getFunction(densities, MOON_RIDGES_FOLDED);
        var primary = getFunction(densities, MOON_PRIMARY);
        ...
        return new NoiseRouter(
                ...
                DensityFunctions.zero(),
                getFunction(densities, MOON_RIDGES),
                        ...
                        DensityFunctions.add(
                                ...
                                DensityFunctions.add(
                                        DensityFunctions.add(
                                                ...
                                                DensityFunctions.yClampedGradient(-64, 256, 2, -3)
                                        ),
                                        variance
                                )
                        )
                                        ...
                                        DensityFunctions.rangeChoice(continents, -0.05f, 0.01f,
                                                DensityFunctions.add(
                                                        ...
                                                        DensityFunctions.mul(// maximize effects away from the transition interfaces
                                                                ...
                                                                DensityFunctions.shiftedNoise2d(shiftX, DensityFunctions.constant(0), 1, noises.getOrThrow(NoiseData.MOON_JAGGEDNESS))
                                                        )
                                                ).clamp(0, 1),
                                                DensityFunctions.rangeChoice(continents, 0, 100,
                                                        DensityFunctions.constant(1),
                                        ...
                                        DensityFunctions.add(
                                                ...
                                                DensityFunctions.add(
                                                        ...
                                                        DensityFunctions.mul(
                                                                ...
                                                                DensityFunctions.shiftedNoise2d(DensityFunctions.constant(0), shiftZ, 1, noises.getOrThrow(NoiseData.MOON_JAGGEDNESS))
                                                        )
                                                )
                                        )
                                )
